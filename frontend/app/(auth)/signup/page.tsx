...
function getFirebaseError(code: string): string {
  switch (code) {
    ...
    case "auth/popup-blocked":
      return "Popup blocked — redirecting to sign-in…"
...
export default function SignUpPage() {
  ...
  useEffect(() => {
    ...
    getAuthRedirectResult()
      .then(async (result) => {
        ...
        if (data.is_new_user) setIsAccountCreated(true)
        else { setAccountExists(true); setIsRedirectLoading(false) }
      })
      .catch((err) => {
  ...
  async function handleSubmit(e: { preventDefault(): void }) {
    ...
    try {
      ...
      if (err instanceof ApiError) {
        ...
        if (msg.includes("already") || msg.includes("in use") || msg.includes("email-already-exists")) {
  ...
  async function handleOAuth(
    ...
    try {
      ...
      if (data.is_new_user) setIsAccountCreated(true)
      else { setAccountExists(true); setLoading(false) }
    } catch (err) {
  ...
  if (isAccountCreated) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#fefae0] px-6">
        <div className="w-full max-w-sm text-center">
          <div className="flex items-center justify-center w-16 h-16 rounded-full bg-[#606c38] mx-auto mb-6">
            <svg className="w-8 h-8 text-white" fill="none" stroke="currentColor" strokeWidth={3} viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
            </svg>
          </div>
          <h1 className="text-2xl font-bold text-[#283618]">Account created!</h1>
          <p className="text-[#606c38] text-sm mt-2">
            Your CVFlow account is ready. Let&apos;s set it up.
          </p>
          <Link
            href="/onboarding"
            className="mt-8 inline-flex items-center justify-center w-full bg-[#dda15e] text-[#283618] font-bold rounded-full py-3 hover:bg-[#bc6c25] transition-colors"
          >
            {"Start onboarding \u2192"}
          </Link>
        </div>
      ...
      <div className="flex-1 flex items-center justify-center bg-[#fefae0] px-6 py-12">
        <div className="w-full max-w-md">
          ...
          <div className="flex items-center gap-4 my-6">
            ...
            <span className="text-xs text-[#606c38]/40">or sign up with email</span>
            <div className="flex-1 h-px bg-[#283618]/10" />
          </div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <Select value={country} onValueChange={setCountry}>
                <SelectTrigger className="rounded-xl bg-white border-[#606c38]/15 h-11 w-full text-[#283618] focus:border-[#606c38] focus:ring-[#606c38]/20">
                  <SelectValue placeholder="Select your country" />
                </SelectTrigger>
                <SelectContent className="bg-white rounded-xl border-[#606c38]/15">
                  {countries.map((c) => (
                    <SelectItem
                      key={c.value}
                      value={c.value}
            ...
            >
              ...
              {isFormLoading ? "Creating account…" : "Create my account →"}
            </button>

            <p className="text-xs text-[#606c38]/40 text-center mt-1">
